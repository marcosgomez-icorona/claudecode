...
$caniero = !empty($usuario_get) ? base64_decode($usuario_get) : '';
...
$canieros = obtiene_canieros($caniero, $desde, $hasta);
...
<div class="card shadow-sm mb-4">    
    ...
    <div class="card-header bg-secondary text-white d-flex flex-column flex-sm-row justify-content-between align-items-center">
        <h4 class="mb-2 mb-sm-0">Ordenes de Compra de Azucar</h4>
    ...
    <div class="card mb-3 shadow">
      <div class="card-body">
        <form method="GET" action="home.php">
            <input type="hidden" name="accion" value="ver_oc_azucar">
            ...
            <div class="row g-2 align-items-end">                                
                <div class="col-12 text-end">
                    <button class="btn btn-sm btn-success" onclick="exportarExcel('oc_azucar', event)">Exportar a Excel</button>                    
                </div> 
            </div>
        </form>
      </div>
    ...
    foreach ($canieros as $nomcaniero):
        $oc_azucar_data = OCAzucarCaniero($nomcaniero['razon_social']);
    ?>
    <div class="card-body"> 
            <div class="table-responsive">
                <table id="oc_azucar" class="table table-striped table-hover table-bordered align-middle">
                    ...
                    <?php if (!empty($canieros)): ?>
                        <?php foreach ($oc_azucar_data as $array): ?>
                        <tr>
                        <td><?php echo isset($array['fecha_emision']) ? htmlspecialchars(fechaDma($array['fecha_emision'])) : ''; ?></td>
                        <td><?php echo htmlspecialchars($array['nro_oc'] ?? ''); ?></td>           
                        <!--                 
                        <td><?php //echo htmlspecialchars($array['plazo_entrega'] ?? ''); ?></td>                            
                        <td><?php //echo htmlspecialchars($array['condicion_pago'] ?? ''); ?></td> 
                        -->
