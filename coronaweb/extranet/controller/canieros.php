 ...
 */
 ...
 function getDetalleCania(string $caniero, string $desde, string $hasta): ?array { // <-- Cambiado a devolver ?array (puede ser null)
    include_once 'conexiones/conexion.php';
    ...
    if(!empty($desde) and !empty($hasta)){
        $periodo=" and  date(fechaindustrial) between '$desde' and '$hasta'"; 
        $limit='';
    }else{
        /*$periodo= " and  date(fechaindustrial)= ( SELECT MAX(DATE(dc.fechaindustrial)) FROM detalle_canieros dc 
                                                                    INNER JOIN canieros c ON dc.razon_social = c.razon_social where  c.razon_social='$caniero')";
        */
         $periodo='';$limit = ' LIMIT 100';                                                           
    }
    ...
    $sql= "SELECT  pesada, detalle_canieros.id_caniero, detalle_canieros.razon_social, detalle_canieros.grupo, fechaindustrial, remito, tipo, bruto_tn, trash, neto_tn, brix, 
            ...
            WHERE detalle_canieros.razon_social = '$caniero' $periodo -- AND YEAR(DATE(fechaindustrial))= YEAR(NOW())
            ORDER BY  pesada DESC $limit;";
    //echo $sql;
    $stmt = $mysqli->prepare($sql);
