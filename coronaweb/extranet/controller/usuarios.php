<?php
    ...
    function alta_usuario($usuario, $password,$nombre,$tipo) {
        ...
        $sql_usuarios = "SELECT (MAX(id_usuario) + 1) 'nuevo_id' FROM usuarios";                    
        $usuarios=$mysqli->query($sql_usuarios) or die(mysqli_error($mysqli));
        $usuario_db=$usuarios->fetch_assoc();
        $nuevo_id=$usuario_db['nuevo_id'];

        $sql_usuario = "INSERT INTO usuarios (id_usuario,usuario,password,nombre,tipo) VALUES ('$nuevo_id','$usuario','$hashed_password','$nombre','$tipo')";                    
