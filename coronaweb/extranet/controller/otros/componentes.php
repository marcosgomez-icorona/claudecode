...
function maestro_componentes($buscar){
    include_once 'conexiones/conexion.php';
    $mysqli=conexion_db();
    ...
    if(!empty($mysqli)){
        ...
        $sql_componentes = "SELECT * FROM componentes ".$where." ORDER BY componentes.descripcion ASC;"; 
...
function listado_componentes($id_equipo){
    include_once 'conexiones/conexion.php';
    $mysqli=conexion_db();

    if(!empty($mysqli)){
        if($id_equipo<>-1){
            $where="WHERE idequipo=".$id_equipo." ";
        }else{
            $where="";
        }
        $sql_componentes = "SELECT id_componente, idproducto, codproducto, componentes.descripcion 'componente', cantidad,equipos.codigoequipo, equipos.descripcion 'equipo', 
                            ...
                            FROM componentes INNER JOIN equipos on componentes.idequipo= equipos.id_equipo
...
function listado_componentes_equipos($buscar){
    include_once 'conexiones/conexion.php';
    $mysqli=conexion_db();
...
function modificar_componente_equipo($id_componente,$cantidad){
    include_once 'conexiones/conexion.php';
        ...
        if(mysqli_error($mysqli)==null){
            echo '<script>
                        formulario = opener.document.getElementById("form-list-componetes-equipos");
