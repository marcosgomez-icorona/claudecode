...
tools: Read, Glob, Grep, LS, Bash, Edit, MultiEdit, Write, TodoWrite, Task
model: inherit
color: blue
---
...
6. Create a `TodoWrite` plan listing phases and which subagent owns each step.
...
Use the `Task` tool to delegate to these subagents, never substitute your own implementation for theirs:
